...
@dataclasses.dataclass(frozen=True)
class ModelInputForEagle(ModelInputForGPUWithSamplingMetadata):
    """
    Used by the EagleModelRunner.
    """
    ...
    def as_broadcastable_tensor_dict(self) -> Dict[str, Any]:
        tensor_dict = {
            "input_tokens": self.input_tokens,
            "input_positions": self.input_positions,
            "input_hidden_states": self.input_hidden_states,
            "lora_requests": self.lora_requests,
            "lora_mapping": self.lora_mapping,
            "multi_modal_kwargs": self.multi_modal_kwargs,
        }
...
class EagleModelRunner(GPUModelRunnerBase[ModelInputForEagle]):
    """
    GPU model runner with sampling step.
    """
    _model_input_cls: Type[ModelInputForEagle] = (
        ModelInputForEagle)
    ...
    def make_model_input_from_broadcasted_tensor_dict(
        ...
        return (
            ModelInputForEagle.from_broadcasted_tensor_dict(
                tensor_dict,
                attn_backend=self.attn_backend,
            ))
    ...
    def prepare_model_input(
        ...
        """
        model_input = self._prepare_model_input_tensors(
            seq_group_metadata_list, input_hidden_states)
        sampling_metadata = SamplingMetadata.prepare(seq_group_metadata_list,
                                                     model_input.seq_lens,
                                                     model_input.query_lens,
                                                     self.device,
    ...
    def execute_model(
        self,
        model_input: ModelInputForEagle,
        kv_caches: List[torch.Tensor],
    ) -> SamplerOutput:
        if self.lora_config:
            assert model_input.lora_requests is not None
        ...
        hidden_states = model_executable(
            input_ids=model_input.input_tokens,
            positions=model_input.input_positions,
            input_hidden_states=model_input.input_hidden_states,
            kv_caches=kv_caches,
            attn_metadata=model_input.attn_metadata,
            **multi_modal_kwargs,
        )
        ...
        # FIXME: (chenzhengda) hard core for eagle draft return_hidden_states 
    ...
    def capture_model(self, kv_caches: List[torch.Tensor]) -> None:
        ...
        input_tokens = torch.zeros(max_batch_size, dtype=torch.long).cuda()
        input_positions = torch.zeros(max_batch_size, dtype=torch.long).cuda()
        input_hidden_states = torch.zeros((max_batch_size, self.model_config.get_hidden_size()), dtype=torch.long).cuda()
        slot_mapping = torch.empty(max_batch_size, dtype=torch.long).cuda()
        slot_mapping.fill_(_PAD_SLOT_ID)
        seq_lens = torch.ones(max_batch_size, dtype=torch.int32).cuda()
        block_tables = torch.from_numpy(self.graph_block_tables).cuda()
        ...
        with graph_capture() as graph_capture_context:
            ...
            for batch_size in reversed(batch_size_capture_list):
                ...
                graph_runner = EagleCUDAGraphRunner(self.model,
                                               self.attn_backend.get_name())
                ...
                graph_runner.capture(
                    input_tokens[:batch_size],
                    input_positions[:batch_size],
                    input_hidden_states[:batch_size],
                    hidden_states[:batch_size]
                    if hidden_states is not None else None,
                    kv_caches,
                    attn_metadata,
...
class EagleCUDAGraphRunner:
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.input_buffers["input_ids"].copy_(input_ids, non_blocking=True)
        self.input_buffers["positions"].copy_(positions, non_blocking=True)
        self.input_buffers["input_hidden_states"].copy_(input_hidden_states, non_blocking=True)
        self.input_buffers["slot_mapping"].copy_(attn_metadata.slot_mapping,
                                                 non_blocking=True)
        if self.backend_name != "flashinfer":
            self.input_buffers["seq_lens_tensor"].copy_(
