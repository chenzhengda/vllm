...
def set_context_value(value):
    ...
    if not hasattr(_hook_context, "async_operations"):
        _hook_context.async_operations = []
...
def add_async_operation(op):
    """Add async operation to thread local storage"""
    if not hasattr(_hook_context, "async_operations"):
        _hook_context.async_operations = []
    _hook_context.async_operations.append(op)


def get_async_operations():
    """Get all async operations"""
    return getattr(_hook_context, "async_operations", [])
...
def register_model_PD_disagg_hooks(model: torch.nn.Module,
    ...
    def pre_forward_hook(module, args, kwargs):
        # print(f"pre_forward_hook: {kwargs}")
        if 'input_ids' not in kwargs:
            raise ValueError("No input_ids tensor found in kwargs.")
        if 'positions' not in kwargs:
            raise ValueError("No positions tensor found in kwargs.")
    ...
    def post_forward_hook(module, args, kwargs, output):
        ...
        async_ops = get_async_operations()
        for future in async_ops:
            try:
                future.wait()
            except Exception as e:
                print(f"async kv cache transfer failed: {e}")
        
        print(f" all kv cache transfer done !!!!")
        clear_async_operations()
...
def register_decoder_layer_PD_disagg_hooks(module: torch.nn.Module,
    ...
    def create_decoderlayer_hook(idx):
        ...
        def decoderlayer_hook(module, args, kwargs, output):
            ...
            torch_thread = threading.Thread(target=_async_send)
            torch_thread.start()
            
            add_async_operation(future)
